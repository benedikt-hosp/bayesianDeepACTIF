...
# Optimized version? or wrong version?
# import math
# import random
#
# import numpy as np
...
#     IQR = Q3 - Q1
#
#     # Directly filter rows that have any column outside the IQR range
#     return df[~((df < (Q1 - 1.5 * IQR)) | (df > (Q3 + 1 * IQR))).any(axis=1)]
#
#
# # def remove_outliers_in_labels(df, window_size, threshold, target_column_name):
# #     # Check if 'Gt_Depth' column exists
...
# #     return df_cleaned
#
# def remove_outliers_in_labels(df, window_size, threshold, target_column_name='Gt_Depth'):
#     """Removes label outliers using a rolling mean threshold."""
#     if target_column_name not in df.columns:
#         raise ValueError(f"Column '{target_column_name}' not found in the DataFrame")
#
#     # Compute rolling mean
#     rolling_mean = df[target_column_name].rolling(window=window_size, center=True).mean()
#
#     # Mask outliers
#     mask = np.abs(df[target_column_name] - rolling_mean) > threshold
#
#     # Remove outliers
#     df_cleaned = df[~mask].reset_index(drop=True)
#
#     print(f"Removed {mask.sum()} outliers from dataset.")
#     return df_cleaned
#
#
# # def remove_outliers_in_labels_fast(df_chunk, window_size, threshold, target_column_name):
...
#
# def getAngle(row):
#     """Computes the angle between right and left gaze direction vectors."""
#     vecR = np.array([row['World_Gaze_Direction_R_X'], row['World_Gaze_Direction_R_Y'], row['World_Gaze_Direction_R_Z']])
#     vecL = np.array([row['World_Gaze_Direction_L_X'], row['World_Gaze_Direction_L_Y'], row['World_Gaze_Direction_L_Z']])
#
#     # Compute norms
#     norm_R, norm_L = np.linalg.norm(vecR), np.linalg.norm(vecL)
#
#     # Avoid division by zero
#     if norm_R < 1e-8 or norm_L < 1e-8:
#         return np.nan
#
#     # Compute angle using dot product
#     cos_angle = np.clip(np.dot(vecR, vecL) / (norm_R * norm_L), -1.0, 1.0)
...
# #
# # def createFeatures(data_in, isGIW=False):
# #     """Generates additional features for gaze analysis."""
# #     # if isGIW:
# #     #     data_in['Vergence_Angle'], data_in['Vergence_Depth'] = zip(
# #     #         *data_in.apply(lambda row: getEyeVergenceAngle_GIW(row, row['IPD']), axis=1))
# #     # else:
...
# #
# #     if isGIW:
# #         # Compute features for GIW setting
# #         data_in['Vergence_Angle'], data_in['Vergence_Depth'] = zip(
# #             *data_in.apply(lambda row: getEyeVergenceAngle_GIW(row, row['IPD']), axis=1))
# #     else:
# #         # Compute standard vergence features
# #         data_in['Vergence_Angle'], data_in['Vergence_Depth'] = zip(*data_in.apply(getEyeVergenceAngle, axis=1))
# #
# #     # ✅ Ensure 'Gaze_Point_Euclidean_Distance' is computed in **both cases**
# #     if 'Gaze_Point_Euclidean_Distance' not in data_in.columns:
# #         data_in['Gaze_Point_Euclidean_Distance'] = np.sqrt(
# #             (data_in['World_Gaze_Direction_R_X'] - data_in['World_Gaze_Direction_L_X']) ** 2 +
# #             (data_in['World_Gaze_Direction_R_Y'] - data_in['World_Gaze_Direction_L_Y']) ** 2
# #         )
# #
# #     # Normalize depth
# #     data_in['Normalized_Depth'] = (data_in['Vergence_Depth'] - data_in['Vergence_Depth'].min()) / \
# #                                   (data_in['Vergence_Depth'].max() - data_in['Vergence_Depth'].min())
# #
# #     # Compute directional magnitude vectorized
# #     gaze_R = data_in[['World_Gaze_Direction_R_X', 'World_Gaze_Direction_R_Y', 'World_Gaze_Direction_R_Z']].to_numpy()
# #     gaze_L = data_in[['World_Gaze_Direction_L_X', 'World_Gaze_Direction_L_Y', 'World_Gaze_Direction_L_Z']].to_numpy()
# #
# #     data_in['Directional_Magnitude_R'] = np.linalg.norm(gaze_R, axis=1)
# #     data_in['Directional_Magnitude_L'] = np.linalg.norm(gaze_L, axis=1)
# #
# #     # Compute angle differences using dot product
# #     dot_product = (gaze_R * gaze_L).sum(axis=1)
# #     magnitudes = data_in['Directional_Magnitude_R'] * data_in['Directional_Magnitude_L']
# #     data_in['Gaze_Vector_Angle'] = np.degrees(np.arccos(np.clip(dot_product / magnitudes, -1.0, 1.0)))
# #
# #     # Compute relative changes and ratios
# #     data_in['Relative_Change_Vergence_Angle'] = data_in['Vergence_Angle'].diff().fillna(0)
# #     data_in['Ratio_Directional_Magnitude'] = data_in['Directional_Magnitude_R'] / data_in['Directional_Magnitude_L']
# #
# #     # Drop NaNs
# #     data_in = data_in.dropna().reset_index(drop=True)
# #
# #     return data_in
#
#
...
# #     return balanced_df
#
# def binData(df, isGIW=False, num_bins=60):
#     """Balances dataset by binning and resampling."""
#     df['Gt_Depth_bin'] = pd.cut(df['Gt_Depth'], bins=num_bins, labels=False)
#
#     # Calculate target bin size (mean bin count)
#     mean_bin_size = df['Gt_Depth_bin'].value_counts().mean()
#
#     # Resample each bin
#     resampled_data = df.groupby('Gt_Depth_bin', group_keys=False).apply(
#         lambda x: x.sample(n=int(mean_bin_size), replace=len(x) < mean_bin_size, random_state=123)
#     )
#
#     # Drop bin column and reset index
#     resampled_data = resampled_data.drop(columns=['Gt_Depth_bin']).reset_index(drop=True)
#
