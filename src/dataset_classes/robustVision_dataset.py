...
class RobustVisionDataset(AbstractDatasetClass):
    ...
    #             df_test = pd.read_csv(depthCalib_path, delimiter="\t", nrows=1)
    #             available_columns = df_test.columns.str.strip().str.replace(' ', '_')  # Normalize CSV headers
    #
    #             # Normalize expected feature names (convert underscores to spaces)
    #             expected_features = [col.replace('_', ' ') for col in self.current_features]
    #
    #             # Find the actual column names that match expected ones
    #             column_mapping = {expected: actual for expected, actual in zip(expected_features, df_test.columns) if expected in df_test.columns}
    #             filtered_features = [column_mapping[expected] for expected in expected_features if expected in column_mapping]
    #
    #             # Read only the matching columns
    #             df = pd.read_csv(depthCalib_path, delimiter="\t", usecols=filtered_features)
    #
    #             # Rename columns to match `self.current_features`
    #             df.rename(columns={v: k.replace(' ', '_') for k, v in column_mapping.items()}, inplace=True)
    #
    ...
    #     self.subject_list = self.input_data['SubjectID'].unique()
    #
    #     print(f"Finished loading input data with {self.input_data.shape[0]} rows.")
    #     return self.input_data
    ...
    #     data_loader = create_dataloaders_dataset(features_tensor, targets_tensor, is_train, batch_size)
    #
    #     return data_loader
